...
To further demonstrate the usefulness of the MEI and Verovio, I'll present next a quote from the article relative to Piano Precision (PP) a GUI program that we are going to explore more in depth in the Score-Alignment section:
...
One such example is Piano Precision, an SV based GUI tool for visualizing score-aligned spectrograms.
...
Piano Precision \citep{@jiangPIANOPRECISIONADVANCING2025} builds on SV by focusing on visualization, evaluation and annotation of musical and audio data aligned with a given musical performance score. By doing this, data annotations can be mapped not only to a given audio (what SV was built on for), but also map annotations that are linked to a score. The authors note the usefulness of this tool for performance researchers, however, understandably it should prove a useful tool for other MIR research fields. 
...
Also, there has been a growing interest in developing web-based MIR applications. Because of this the usage of HTML and JS based codebases might also be found. 
...
% Opá, eu quero despachar isto... será que vale a pena eu explicar os metodos algoritmicos também? A minha tese é focada na visualização....
...
We began by categorizing music visualizations into three main types: traditional sheet music representations, symbolic machine-readable formats, and audio-based representations. The discussion of symbolic representations highlighted the critical importance of the MEI format and Verovio, which together enable rich, semantically meaningful digital score encodings that preserve both musical structure and metadata. This connection between MEI's machine-readable encoding and Verovio's interactive SVG rendering proved essential for understanding modern score visualization workflows.
