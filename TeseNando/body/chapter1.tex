...
MIR research increasingly demands visualisations that integrate multiple modalities simultaneously. MIR researchers often need to inspect algorithmic outputs alongside audio representations and symbolic scores in a time-coherent manner. This kind of multi-modal, time-aligned visualisation is central to both the evaluation of MIR algorithms and the development of annotation workflows. Audio-to-Score Alignment (i.e., the task of synchronizing a performance recording with its corresponding symbolic score) is a key enabler of this type of visualisation, as it provides the temporal anchor needed to align heterogeneous data sources into a unified view.

% Focus on BT/BA
This thesis was initiated by two closely related challenges within \ac{BT} and \ac{BA} research. Contemporary BT systems are predominantly deep learning-based thus, the field is deeply reliant on tools to enable better execution of annotation and evaluation tasks, these are essential for advancing BT research. 
...
\begin{itemize}
    \item \textbf{Objective 1}: Develop a reusable Python framework for composing score-aligned MIR visualisations based on an object-oriented layer-composition model.
    
    \item \textbf{Objective 2}: Demonstrate that the framework supports visualisation workflows that are difficult or impractical with current MIR tools.
...
\begin{itemize}
    ...
    \item \textbf{RQ2}: To what extent does the proposed framework simplify the creation, modification, and extension of score-aligned visualisations compared with manual SVG editing, Sonic Visualiser, and Piano Precision?
    \item \textbf{RQ3}: How can score-aligned visualisations support beat annotation and beat-tracking evaluation workflows, and under what conditions does that support break down?
\end{itemize}

The main contributions of this thesis are the following:
\begin{itemize}
    \item The LayerIt framework for composing score-aligned MIR visualisations through an object-oriented layer-composition model.
...
To address the previously mentioned objectives and RQ, we developed LayerIt!, a Python-based object-oriented framework for composing score-aligned MIR visualisations. LayerIt integrates established MIR libraries and tools into an SVG-based workflow in which visual elements are treated as independent layers that can be composed, reordered, and combined into custom figures.
...
\begin{itemize}
    \item \textbf{Chapter~\ref{chap:sota}} provides a literature review covering the main audio visualisation types, along with existing \ac{GUI} and code-based visualisation tools, and the task of Audio-to-Score Alignment.
    ...
    \item \textbf{Chapter~\ref{chap:Validation}} is dedicated to describing the validation of this thesis and project. This chapter should give some closure to the RQ that are outlined in this introduction chapter.
