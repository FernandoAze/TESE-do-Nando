...
\begin{itemize}
    ...
    \item Other researchers may rely on slicing, cropping, combining and drawing visual elements manually. From SVGs, PNGs to digitalized PDFs there is no one unified and established framework in the MIR community for producing meaningful time-aligned visualizations. 
\end{itemize}
...
\begin{enumerate}
    ...
    \item Integration of ML self-supervised and human-in-the-loop features that provide intelligent annotation suggestions and improve user experience (e.g., Yamamoto et al., 2021 \cite{yamamotoHumanintheLoopAdaptationInteractive2021}).
...
The project bases itself in librosa \cite{mcfeeLibrosaLibrosa01102025}, matplotlib \cite{thematplotlibdevelopmentteamMatplotlibVisualizationPython2026} and modusa \cite{modusa} to generate audio-visualizations and data plots. The score used for the symbolic representation layer is rendered through ScoreWarp. The end result is an SVG file that overlays the audio-visualization, data layers and score. The codebase also integrates BeatThis the as-of-now SOTA BT algorithm. Through its implementation we demonstrate how a given BT model can be integrated to our system in order to produce beat data visualizations.
