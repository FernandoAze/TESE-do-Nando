...
This chapter is dedicated to presenting and contextualizing the problem we chose to tackle and the subsequent solution we developed to address it. We'll lay out the different iterations we passed through, since each progressively provided insights that led to the core conclusions and value proposition of the thesis.
...
Modern Music Information Retrieval (MIR) tools face a fundamental challenge in visualization: while individual components exist for audio analysis (librosa, matplotlib), score representation (Verovio, ScoreWarp), and algorithmic development (BeatThis and other SOTA methods), there is no unified framework capable of composably aligning these heterogeneous data sources in time. Existing tools like Sonic Visualizer, despite their power and widespread adoption, struggle to incorporate newer algorithmic developments because their visualization components are constrained by legacy architectural decisions. The codebase complexity and entrenched design patterns make experimentation with visual representations and integration of novel algorithms increasingly difficult, creating a bottleneck between SOTA algorithm development and effective visualization.
