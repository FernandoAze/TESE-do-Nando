...
LayerIt is a Python framework designed to compose score-aligned MIR visualizations from independent visual components. At a high level, it takes aligned musical data, generates the corresponding visual representations, and arranges them into a layered figure that preserves the temporal relationship between the score and the accompanying audio-derived or annotation-derived information.

The framework is modular, this means that the same base structure can support different combinations of score material, spectrograms, annotations, beat information, or other derived data without requiring the entire figure to be rebuilt manually. The result is a visualization workflow that is easier to extend than a static editing process and more reproducible than a one-off graphical composition.

The project bases itself in librosa \cite{mcfeeLibrosaLibrosa01102025}, matplotlib \cite{thematplotlibdevelopmentteamMatplotlibVisualizationPython2026} and modusa \cite{modusa} to generate audio and data visualizations. The score layer is rendered through ScoreWarp. The end result is an SVG image file that overlays the audio and data layers, with the score. The codebase also integrates BeatThis the state-of-the-art BT algorithm. Through its implementation we demonstrate how a given BT model can be integrated to our system in order to produce beat data visualizations.
...
% FALTA FALAR DA RAZAO PELA QUAL SAO USADAS OUTRAS BIBLIOTECAS
...
The visualizations generated by LayerIt follow a ``layers within layers'' philosophy: visual elements are self-contained components that can be composed into upper‑level elements, which in turn can be composed into yet higher levels. \autoref{fig:LayersWithinLayers} demonstrates this methodology.
