...
In LayerIt, a \texttt{Visualizer} instance orchestrates the composition of multiple layers through a panel-based workflow. Layers are grouped into panels using \texttt{add_panel()}, which allows multiple layers to be rendered together on the same axis. The \texttt{Visualizer} maintains a shared data dictionary that all layers access during rendering via \texttt{load_all_layers()} and \texttt{draw()}. The \texttt{compose()} method then renders all panels and combines them with the warped score into a final multi-panel SVG. The next listing (\ref{lst:visualizer-api}) illustrates this composition workflow.
...
visualizer.compose("output.svg", 
                   gap=20,
                   score_position=0)
\end{lstlisting}

In the composition workflow, the \texttt{Visualizer.compose()} method orchestrates the rendering of all panels and their combination with the warped score. Internally, each panel is rendered to SVG through \texttt{turn_to_SVG()}, which preserves each layer as a named SVG group. These panel SVGs are then combined with the warped score SVG using \texttt{create_final_SVG()}, which stacks them vertically at defined y-offsets while preserving the temporal alignment through the common timeline. The following illustrative listing shows this internal composition workflow.
