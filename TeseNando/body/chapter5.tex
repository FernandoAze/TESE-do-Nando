...
Nothing outside the new class was changed in the LayerIt framework or in any existing layer implementation: \texttt{Visualizer}, the base \texttt{Layer} and shape classes, and the existing audio and beat layers remained unchanged. Two supporting additions were made only to expose and demonstrate the class: its public export in \texttt{__init__.py} and the separate \texttt{OnsetNoveltyDemo.py} example script. The result is shown in \ref{fig:onset-novelty-demo}.
